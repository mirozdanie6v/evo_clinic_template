...
for (const id of ids) {
  ...
  const company = await get(page, `${api}/company/${id}?forBooking=1&bookform_id=${form}&include=has_cashback`);
  const catalog = await get(page, `${api}/book_services/${id}?without_seances=1`);
  const staff = await get(page, `${api}/book_staff/${id}?datetime=&without_seances=1`);
  const serviceStaff = {};
  for (const service of catalog.services || []) {
    const filtered = await get(page, `${api}/book_staff/${id}?datetime=&without_seances=1&service_ids[]=${service.id}`);
