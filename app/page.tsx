...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const now=new Date();
    setDates(Array.from({length:6},(_,index)=>{
      const d=new Date(now);d.setDate(now.getDate()+index+1);
  ...
  const categoriesForGroup=useMemo(()=>groupFilter==="all"?categories:categories.filter(category=>category.groupId===groupFilter),[groupFilter]);

  const filteredServices=useMemo(()=>{
    const q=normalize(serviceSearch);
    return services.filter(service=>{
      const category=getCategory(service.categoryId);
      if(groupFilter!=="all"&&category?.groupId!==groupFilter)return false;
      if(categoryFilter!=="all"&&service.categoryId!==categoryFilter)return false;
      if(!q)return true;
      const group=category?getGroup(category.groupId):undefined;
      const text=[...Object.values(service.name),...Object.values(service.description),...(category?Object.values(category.name):[]),...(group?Object.values(group.name):[])].join(" ").toLocaleLowerCase();
      return text.includes(q);
    });
  },[serviceSearch,groupFilter,categoryFilter]);

  const bookingServices=useMemo(()=>{
    const q=normalize(bookingSearch);
    return services.filter(service=>{
      const category=getCategory(service.categoryId);
      if(bookingGroupFilter!=="all"&&category?.groupId!==bookingGroupFilter)return false;
      if(!q)return true;
      const group=category?getGroup(category.groupId):undefined;
      return [...Object.values(service.name),...(category?Object.values(category.name):[]),...(group?Object.values(group.name):[])].join(" ").toLocaleLowerCase().includes(q);
    });
  },[bookingSearch,bookingGroupFilter]);

  const visibleCategories=useMemo(()=>categoriesForGroup.filter(category=>filteredServices.some(service=>service.categoryId===category.id)),[categoriesForGroup,filteredServices]);
  ...
  const changeLang=(value:Lang)=>{setLang(value);localStorage.setItem("evo-lang",value);document.documentElement.lang=value};
  ...
  const openCategory=(id:string)=>{const category=getCategory(id);setGroupFilter(category?.groupId||"all");setCategoryFilter(id);setServiceSearch("");setScreen("services");window.scrollTo({top:0,behavior:"smooth"})};
  const openGroup=(id:string)=>{setGroupFilter(id);setCategoryFilter("all");setServiceSearch("");setScreen("services");window.scrollTo({top:0,behavior:"smooth"})};
  ...
  const startBooking=(serviceId="",specialistId="")=>{
    ...
    setBookingSearch("");
    setBookingGroupFilter(category?.groupId||"all");
    setSuccess(false);setScreen("booking");window.scrollTo({top:0,behavior:"smooth"});
  };
  ...
  const submitAppointment=()=>{
    ...
    addAppointment({...booking,id:makeAppointmentId(),createdAt:new Date().toISOString()});
    setAppointments(readAppointments());setSuccess(true);
  };

  const repeatAppointment=(item:DemoAppointment)=>{
    const service=getService(item.serviceId);const category=service?getCategory(service.categoryId):undefined;
    ...
    setBookingGroupFilter(category?.groupId||"all");setBookingStep(2);setSuccess(false);setScreen("booking");
  };
  ...
  const askCatalog=(event?:FormEvent)=>{
    ...
    const scored=services.map(service=>{
      const category=getCategory(service.categoryId);const group=category?getGroup(category.groupId):undefined;
      const haystack=[...Object.values(service.name),...Object.values(service.description),...(category?Object.values(category.name):[]),...(group?Object.values(group.name):[])].join(" ").toLowerCase();
      return {service,score:tokens.reduce((sum,token)=>sum+(haystack.includes(token)?1:0),0)};
  ...
  return <main className="shell">
    ...
    <section className="content">
      {screen==="home"&&<>
        <section className="hero"><div className="heroShade"/><div className="heroText"><small>{t.home.eyebrow}</small><h1>{t.home.title}</h1><p>{t.home.lead}</p><div className="actions"><button className="primary light" onClick={()=>startBooking()}>{t.common.book}</button><button className="secondary light" onClick={()=>{resetCatalog();nav("services")}}>{t.nav.services}</button></div></div></section>
        <button className="status" onClick={()=>startBooking()}><i/><span><b>{t.home.open}</b><small>{t.home.consult}</small></span><em>→</em></button>
        <SectionTitle title={t.home.popular} action={t.home.all} onAction={()=>{resetCatalog();nav("services")}}/>
        <div className="categoryGrid" data-testid="service-group-grid">{groups.map(group=><button className="categoryCard" key={group.id} onClick={()=>openGroup(group.id)}><img src={group.image} alt=""/><span><b>{group.name[lang]}</b><small>{group.note[lang]}</small></span></button>)}</div>
      ...
      {screen==="booking"&&<section className="page"><PageHead eyebrow="EVO BOOKING" title={t.booking.title} lead={t.booking.demo}/>{success?<div className="successCard"><span>✓</span><h2>{t.booking.success}</h2><p>{t.booking.successNote}</p><button className="primary full" onClick={()=>nav("profile")}>{t.nav.profile}</button><a className="secondary full center" href={location.bookingUrl} target="_blank" rel="noreferrer">{t.booking.openAltegio}</a></div>:<div className="bookingFlow"><div className="stepper">{[t.booking.stepService,t.booking.stepSpecialist,t.booking.stepDate,t.booking.stepTime,t.booking.stepContact,t.booking.stepReview].map((label,index)=><span key={label} className={bookingStep===index?"on":bookingStep>index?"done":""}>{index+1}<small>{label}</small></span>)}</div>
        {bookingStep===0&&<div><h2>{t.booking.chooseService}</h2><div className="categoryChips bookingGroups" data-testid="booking-group-filter"><button className={bookingGroupFilter==="all"?"on":""} onClick={()=>setBookingGroupFilter("all")}>{t.common.all}</button>{groups.map(group=><button key={group.id} className={bookingGroupFilter===group.id?"on":""} onClick={()=>setBookingGroupFilter(group.id)}>{group.name[lang]}</button>)}</div><label className="catalogSearch compact"><span>⌕</span><input data-testid="booking-search" value={bookingSearch} onChange={e=>setBookingSearch(e.target.value)} placeholder={t.booking.searchService}/>{bookingSearch&&<button onClick={()=>setBookingSearch("")} type="button" aria-label={t.common.close}>×</button>}</label><div className="choiceList" data-testid="booking-service-list">{bookingServices.map(service=><button key={service.id} onClick={()=>chooseService(service.id)}><img src={service.image} alt=""/><span><b>{service.name[lang]}</b><small>{getCategory(service.categoryId)?.name[lang]} · {service.price[lang]}</small></span><em>→</em></button>)}</div></div>}
